...
module Make(Params : sig
  ...
  let fetch_directive_argument (lexer : lexer) lexbuf
    ...
    let rec loop acc (brackets : Parser.token list) =
      ...
      | EOF, _ | EOL, [] -> (end_pos, located Parser.EOF lexbuf :: acc)
    ...
    in
    let end_pos, tokens = loop tokens [] in
    (end_pos,
  ...
  let parse_directive (lexer : lexer) lexbuf ~in_import : Directive.t Located.t =
    let hash_loc = Location.of_lexbuf lexbuf in
  ...
  let rec next_directive (lexer : lexer) lexbuf ~in_import =
    match lexer lexbuf with
    | HASH -> parse_directive lexer lexbuf ~in_import
    | EOL -> next_directive lexer lexbuf ~in_import
    | EOF -> endif_missing lexbuf
    | _ -> skip_line lexer lexbuf; next_directive lexer lexbuf ~in_import

  let rec next_endif lexer lexbuf ~in_import =
    let dir = next_directive lexer lexbuf ~in_import in
    match dir.txt with
    | If _ -> skip_if lexer lexbuf ~in_import; next_endif lexer lexbuf ~in_import
    | Else
    | Elif _
    | Endif -> dir
    | _     -> next_endif lexer lexbuf ~in_import

  and skip_if lexer lexbuf ~in_import =
    match (next_directive lexer lexbuf ~in_import).txt with
    | If _   -> skip_if lexer lexbuf ~in_import; skip_if lexer lexbuf ~in_import
    | Else   -> skip_else lexer lexbuf ~in_import
    | Elif _ -> skip_if lexer lexbuf ~in_import
    | Endif  -> ()
    | _      -> skip_if lexer lexbuf ~in_import

  and skip_else lexer lexbuf ~in_import =
    match (next_directive lexer lexbuf ~in_import).txt with
    | If _ -> skip_if lexer lexbuf ~in_import; skip_else lexer lexbuf ~in_import
    | Else ->
    ...
    | Endif -> ()
    | _ -> skip_else lexer lexbuf ~in_import
  ...
  let rec lexer_internal (lexer : lexer) lexbuf ~in_import : Parser.token =
    match lexer lexbuf with
    | HASH when at_bol lexbuf ->
      interpret_directive lexer lexbuf ~in_import
        (parse_directive lexer lexbuf ~in_import);
      lexer_internal lexer lexbuf ~in_import
    | EOF -> Stack.check_eof lexbuf; EOF
    | token -> token

  and lexer_internal_skip_eols lexer lexbuf ~in_import =
    match lexer_internal lexer lexbuf ~in_import with
    | EOL -> lexer_internal_skip_eols lexer lexbuf ~in_import
    | token -> token

  and interpret_directive lexer lexbuf ~in_import (dir : Directive.t Located.t) =
    match dir.txt with
    | If e -> interpret_if lexer lexbuf dir e ~in_import
    ...
    | Ifndef var ->
      ...
      if Env.seen !env var then
        interpret_if lexer lexbuf dir e ~in_import
      else begin
        ...
        | HASH when at_bol lexbuf -> begin
            match (parse_directive lexer lexbuf ~in_import).txt with
    ...
    | Else   -> Stack.dequeue dir; skip_else lexer lexbuf ~in_import
    | Elif _ -> Stack.dequeue dir; skip_if   lexer lexbuf ~in_import
  ...
  and import ~loc lexer fname =
    ...
    match lexer_internal_skip_eols lexer lexbuf ~in_import:true with
    | exception e -> begin
  ...
  let map_lexer lexer lexbuf =
    try
      lexer_internal lexer lexbuf ~in_import:false
    with exn ->
      map_exn exn
  ...
  let preprocess_file fn oc =
    ...
    let rec loop pos acc =
      ...
      | HASH when at_bol lexbuf ->
        ...
        interpret_directive Lexer.token lexbuf
          (parse_directive Lexer.token lexbuf ~in_import:false)
          ~in_import:false;
...
let () =
  ...
  Ppx_driver.register_process_file_hook (fun () ->
    if !use_optcomp then begin
      ...
      Lexer.set_preprocessor M.init M.map_lexer;
    end)
